    ...
    public sealed class TcpSocket : ITcpSocket<TcpSocket> {
        ...
        private void Accept() {
            lock (acceptLock) {
                ...
                if (accepted != null) {
                    serverListener?.SocketDidAccept(new TcpSocket(accepted) { hasBeenConnected = true });
                }
            }
        }
            ...
            lock (receiveLock) {
                ...
                if (socket.Poll(1, SelectMode.SelectRead)) {
                    if (CheckDisconnected()) {
                        ...
                        int count = socket.Receive(buffer, 0, Consts.bufferSize, SocketFlags.None, out SocketError error);
                        if (count > 0) { ioListener?.SocketDidReceiveBytes(this, buffer, count); }
                        bufferPool.Pay(buffer);

                        if (CheckSocketError(error)) {
                            NotifyClosing();
                        }
                    }
                }
        ...
        public void Send(byte[] bytes, int count) {
            lock (sendLock) {
                ...
                if (socket.Poll(1, SelectMode.SelectWrite)) {
                    int written = socket.Send(bytes, 0, count, SocketFlags.None, out SocketError error);
