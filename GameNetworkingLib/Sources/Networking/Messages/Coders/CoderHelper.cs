...
namespace GameNetworking.Messages.Coders {
    public static class CoderHelper {
        ...
        public static byte CalculateChecksum(byte[] data, int index, int length) {
            byte checksum = 0;
            unchecked { for (int idx = index; idx < length; idx++) { checksum += data[idx]; } }
            return checksum;
        }
    }
    ...
    public static class ArraySearch {
        public static int IndexOf(byte[] pattern, int patternLength, byte[] inArray, int arrayLength) {
            if (patternLength == 0 || arrayLength == 0 || arrayLength < patternLength) {
                return -1;
            }

            bool missmatchInPattern = false;
            for (int inArrayIndex = 0; inArrayIndex < arrayLength; inArrayIndex++) {
                int patternIndex;
                int currentArrayIndex = inArrayIndex;
                ...
                if (missmatchInPattern) {
                    continue;
                }

                if (patternIndex == patternLength && !missmatchInPattern) {
                    return inArrayIndex;
                }
            }
